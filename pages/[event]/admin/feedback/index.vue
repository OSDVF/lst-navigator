<template>
    <SplitPage :split="split">
        <template #header>
            <h1>Nastavení zpětné vazby</h1>
            <p>
                <NuxtLink :to="`/${cloud.selectedEvent}/admin/feedback/result`">
                    <button class="large">
                        <Icon name="mdi:spreadsheet" class="mb-0.5e" />
                        Výsledky
                    </button>
                </NuxtLink>
                <NuxtLink :to="`/${cloud.selectedEvent}/feedback`">
                    <button class="large">
            ...
            <ImportForm
                v-if="cloud.resolvedPermissions.superAdmin && cloud.feedback.col"
                :collection="cloud.feedback.col" @error="e => error = e" />
            ...
            <template v-if="cloud.resolvedPermissions.editEvent">
                <select value="" @change="e => { copyFrom(e.target as HTMLSelectElement) }">
                    ...
                    <option
                        v-for="event in cloud.eventsCollection.filter(e => e.id != cloud.selectedEvent)"
                        :key="event.id" :value="event.id">
                        {{ event.title }}
                    </option>
                </select>
                <EditableField
                    :parent-level="2" class="p" description="Informace pro respondenty:"
                    property="feedbackInfo" placeholder="Vyplňte všechno jako..." />
            </template>
        </template>
        <template v-if="cloud.resolvedPermissions.editEvent">
            <main class="feedback">
