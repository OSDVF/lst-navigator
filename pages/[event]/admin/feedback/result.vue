<template>
    <LazyClientOnly>
        <Teleport to="#topNav">
            ...
            <NuxtLink
                :to="`/${cloud.selectedEvent}/admin/feedback/result/program`" class="p-1 inline-flex align-items-center tab"
                title="Části programu">
                <Icon name="mdi:calendar" />
                <span class="sm-hide">Části programu</span>
            </NuxtLink>
            <NuxtLink
                :to="`/${cloud.selectedEvent}/admin/feedback/result/other`" class="p-1 inline-flex align-items-center tab"
        ...
        <div :class="'p-1 feedbackResult ' + admin.displayKind">
            <div class="flex flex-wrap justify-content-between p-1">
                <div class="mr-1">
                    <template v-if="admin.displayKind == 'individual'">
                        <template v-if="cloud.user.auth?.uid && cloud.resolvedPermissions.editEvent">
                            <br>
            ...
            <template v-if="typeof cloud.days?.findLast(() => true) === 'string'">
                Načítání
            </template>
            <template v-else>
...
function deleteAll() {
    if(confirm('Chcete smazat všechny odpovědi respondentů u této akce?')) {
        return cloud.feedback.clear()
    }
}

function deleteRespondent(respondent: string) {
    if (confirm(`Opravdu chcete smazat odpovědi respondenta ${respondent}?`)) {
        if (cloud.feedback.online) {
            for (const section in cloud.feedback.online) {
                const sectionData = cloud.feedback.online[section]
                if (typeof sectionData !== 'number' && typeof sectionData !== 'string') {
                    for (const question in sectionData) {
                        if ((sectionData as FeedbackQuestionsCustom)[question]?.[respondent]) {
                            cloud.feedback.set(section, question, null, respondent)
                        }
                    }
                }
            }
