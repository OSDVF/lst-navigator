...
function todayOr0(newCloud: typeof cloud) {
    ...
    router.replace({
        path: `/${cloud.selectedEvent}/schedule/${index}`,
...
onBeforeRouteUpdate((to, from) => {// guard slide direction and go to today when no day is selected
    ...
    if (isNaN(toDay)) {
        ...
        const toItem = parseInt(to.params.item as string)
        const fromItem = parseInt(from.params.item as string)
        if (isNaN(toItem)) {
            ...
            if (!isNaN(fromItem) && toItem < fromItem) {
                currentTransition.value = 'slide-right'
                return
            }
        }
...
if (settings.gestures) {
    ...
    const controller = useDrag(async ({ movement: [x], dragging, swipe }: { movement: number[], dragging: boolean, swipe: Vector2 }) => {
        ...
        const eventIndex = parseInt(router.currentRoute.value.params.item as string)
        ...
        if (swipe[0] !== 0) {
            ...
            if (isNaN(eventIndex)) {
                if (!cantGoLeftDay && !cantGoRightDay) {
                    ...
                    await go(swipe[0], `/${cloud.selectedEvent}/schedule/${intDay - swipe[0]}`)
                    return
                } else {
                    gestureTransPending.value = false
                    reset(false)
                ...
                if (x > 0 && eventIndex === 0 && !cantGoLeftDay) {
                    await go(swipe[0], `/${cloud.selectedEvent}/schedule/${intDay - 1}/${(cloud.days[intDay - 1].program.length - 1 || 0)}`)// last event on the previous schedule par)
                    return
                } else if (x < 0 && eventIndex === lastEventIndex && !cantGoRightDay) {
                    await go(swipe[0], `/${cloud.selectedEvent}/schedule/${intDay + 1}/0`)
                    return
                } else if ((swipe[0] > 0 && eventIndex > 0) || swipe[0] < 0 && eventIndex < lastEventIndex) {
                    // move to next or prev day
                    await go(swipe[0], `/${cloud.selectedEvent}/schedule/${intDay}/${eventIndex - swipe[0]}`)
                    return
                } else {
                    gestureTransPending.value = false
                    reset(false)
