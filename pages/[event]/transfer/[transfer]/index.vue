<template>
    <article>
        ...
        <div v-if="cloud.resolvedPermissions.editEvent && here" class="mt-2">
            ...
            <button
                :disabled="!validUID"
                @click="validUID && router.replace(`/${cloud.selectedEvent}/transfer/admin/${adminOtherUID}`)">Přenést</button><br>
...
watch([canvas, router.currentRoute], async ([newCanvas, newTarget]) => {
    if (newCanvas) {
        qrcode.toCanvas(newCanvas, new URL(`/${cloud.selectedEvent}/transfer/${newTarget.params.transfer}/${settings.userIdentifier}`, location.origin).toString(), { errorCorrectionLevel: 'H' }, (e) => {
            if (e) {
                console.error(error)
                error.value = e
...
watch(transfer, async (newTransfer) => {
    if (here.value) {
        ...
        if (newTransfer?.status == TransferStatus.Transfered) {
            setTimeout(async () => {
                router.replace(`/${cloud.selectedEvent}/settings`)
                await setDocT(doc, {
                    status: TransferStatus.None,
                })
            }, 1500)
