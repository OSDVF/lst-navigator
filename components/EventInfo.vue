<template>
    <article role="article">
        ...
        <h1>{{ cloud.eventDescription?.title }}<NuxtLink
            ...
            <NuxtLink class="button small" style="float:right" to="/">
                <Icon name="mdi:apps" size="1.5rem" /><Icon name="mdi:arrow-left" />&ensp;Všechny události
            </NuxtLink>
        </h1>
        ...
        <h6>
            ...
            <template v-if="applicationsEnabled(cloud.eventDescription)">
                ...
                <a :href="cloud.eventDescription!.form" target="_blank" rel="noopener noreferrer">
                    <Icon mode="svg" name="mdi:form-select" size="1rem" />&ensp;Online přihláška
                </a>
            </template>
            &ensp;
        </h6>
